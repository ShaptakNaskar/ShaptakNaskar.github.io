...
function App() {
  ...
  return (
    <div className="min-h-screen transition-colors duration-300 relative overflow-hidden font-sans">
      <ScrollToHashElement />
      <ScrollToTopButton />
      {/* Background with abstract blobs - Consistent across pages but hidden for standalone */}
      {!isStandaloneApp && (
        ...
        <main className="relative z-10 w-full h-full min-h-screen">
          <AnimatePresence mode="wait">
            <Routes location={location} key={location.pathname}>
              <Route
                path="/webapps/weather"
                element={
                  <motion.div
                    initial={{ opacity: 0 }}
                    animate={{ opacity: 1 }}
                    exit={{ opacity: 0 }}
                    transition={{ duration: 1.5 }} // Longer fade to black transition
